-- Phase 1: Add missing fields and relationships to master data tables (Fixed)

-- 1. First, add unique constraint to master_countries name column
ALTER TABLE public.master_countries 
ADD CONSTRAINT unique_master_countries_name UNIQUE (name);

-- 2. Add country field to master_currencies 
ALTER TABLE public.master_currencies 
ADD COLUMN country text,
ADD COLUMN country_code text;

-- 3. Add foreign key constraint linking currencies to countries (using name)
ALTER TABLE public.master_currencies 
ADD CONSTRAINT fk_master_currencies_country 
FOREIGN KEY (country) REFERENCES public.master_countries(name) ON UPDATE CASCADE;

-- 4. Ensure proper foreign key relationship for master_sub_categories
...
-- 5. Add foreign key constraint for master_domain_groups to industry_segments
...
-- 7. Add missing fields to master_seeker_membership_fees for better structure
...
-- 8. Add validation function for currency-country consistency
...
-- 9. Add validation for pricing configs to ensure proper fee structure
