-- Remove the overly restrictive constraint on fee components
ALTER TABLE master_fee_components 
DROP CONSTRAINT IF EXISTS master_fee_components_component_type_check;

-- Add back the missing fee component types
INSERT INTO master_fee_components (name, component_type, description, is_active, is_user_created, created_at, updated_at)
VALUES 
  ('Total Fee', 'total_fee', 'Total comprehensive fee calculation', true, false, now(), now()),
  ('Platform Usage Fee', 'platform_usage_fee', 'Fee for platform usage and services', true, false, now(), now()),
  ('Advance Payment', 'advance_payment', 'Advance payment component', true, false, now(), now())
