-- Enhanced Membership Workflow Schema
...
-- Add check constraints for valid workflow steps
ALTER TABLE engagement_activations 
ADD CONSTRAINT IF NOT EXISTS chk_workflow_step 
CHECK (workflow_step IN ('membership_decision', 'payment', 'tier_selection', 'engagement_model', 'details_review', 'activation_complete'));

-- Add check constraints for valid payment simulation status
ALTER TABLE engagement_activations 
ADD CONSTRAINT IF NOT EXISTS chk_payment_simulation_status 
CHECK (payment_simulation_status IN ('pending', 'processing', 'success', 'failed'));

-- Add check constraints for valid pricing tiers
ALTER TABLE engagement_activations 
ADD CONSTRAINT IF NOT EXISTS chk_pricing_tier 
